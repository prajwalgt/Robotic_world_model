...
class CassieManagerBasedMBRLEnv(ManagerBasedMBRLEnv):
    ...
    def get_imagination_observation(self, state_history, action_history, observation_noise=True):
        obs_state = self.imagination_state_normalizer.inverse(state_history[:, -1])
        obs_base_lin_vel = obs_state[:, 0:3]
        obs_base_ang_vel = obs_state[:, 3:6]
        obs_projected_gravity = obs_state[:, 6:9]
        obs_joint_pos = obs_state[:, 9:21]
        obs_joint_vel = obs_state[:, 21:33]
    ...
    def _compute_imagination_reward_terms(self, parsed_imagination_states, rollout_action, parsed_extensions, parsed_contacts):
        ...
        if toe_contact is not None:
            ...
            self.last_contact_time = torch.where(is_first_detached, self.current_contact_time + self.step_dt, self.last_contact_time)
        ...
        undesired_contacts = (
            torch.sum(ankle_contact, dim=1) if ankle_contact is not None else torch.zeros(self.num_imagination_envs, device=self.device)
        )
        flat_orientation_l2 = torch.sum(torch.square(projected_gravity[:, :2]), dim=1)
        joint_deviation = torch.sum(torch.abs(joint_pos), dim=1)
        termination_penalty = (
            self.termination_flags.float() if self.termination_flags is not None else torch.zeros(self.num_imagination_envs, device=self.device)
        )
        ...
        if toe_height is not None and toe_planar_velocity is not None:
            ...
            foot_clearance = torch.zeros(self.num_imagination_envs, device=self.device)
