...
from rsl_rl.modules.architectures import MLPBase, RNNBase, TransformerBase, DecoderTransformerBase, MLPStateHead, MLPAuxiliaryHead
...
class SystemDynamicsEnsemble(nn.Module):
    ...
    def _create_base(self):
        ...
        elif self.architecture_config["type"] == "decoder_transformer":
            input_dim = self.state_dim + self.action_dim
            self.base_output_dim = self.architecture_config.get("decoder_transformer_d_model", 64)
            self.prediction_type = "single"
            return DecoderTransformerBase(
                input_dim=input_dim,
                device=self.device,
                architecture_config=self.architecture_config,
            )
